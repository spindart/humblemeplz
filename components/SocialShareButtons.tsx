...
export const SocialShareButtons: React.FC<SocialShareButtonsProps> = ({ analysisText }) => {
  const getPlatformMessage = (platform: 'twitter' | 'linkedin' | 'facebook' | 'whatsapp') => {
    if (platform === 'twitter') {
  ...
  const handleShare = (platform: 'twitter' | 'linkedin' | 'facebook' | 'whatsapp') => {
    const shareText = getPlatformMessage(platform);
    ...
    switch (platform) {
      case 'twitter':
        ...
        break;
      case 'linkedin':
        const linkedinSummary = encodeURIComponent("Just got my resume brutally analyzed on HumbleMePlz. Humbling, hilarious, and a must-try! #HumbleMePlz");
        shareUrl = `https://www.linkedin.com/sharing/share-offsite/?url=${websiteUrl}&summary=${linkedinSummary}`;
        break;
      case 'facebook':
  ...
  const copyToClipboard = () => {
    const shareText = getPlatformMessage('facebook'); // Use the full text for Instagram
  ...
  return (
    <div className="flex flex-wrap justify-center gap-2 mt-4 mb-4">
      ...
      <motion.button
        ...
        className="flex items-center px-3 py-2 bg-[#C13584] text-white rounded-md hover:bg-opacity-90 transition-colors"
